...
function EditUserDialog({
  ...
  useEffect(() => {
    if (user) {
      ...
      setErpEmployeeId(user.erpEmployeeId || "");
      setError("");
    }
  }, [user]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await customFetch(`/api/admin/users/${user!.id}`, {
        method: "PATCH",
        body: JSON.stringify({
          firstName,
          lastName,
      ...
      toast({ title: `Updated details for ${firstName || email}` });
      onUpdated();
      onClose();
    } catch (err: any) {
    ...
    <Dialog open={!!user} onOpenChange={(v) => { if (!v) onClose(); }}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Update details and ERP mapping for <strong>{user ? getDisplayName(user) : ""}</strong>.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Input id="eu-firstName" value={firstName} onChange={(e) => setFirstName(e.target.value)} placeholder="First Name" />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="eu-lastName">Last Name</Label>
              <Input id="eu-lastName" value={lastName} onChange={(e) => setLastName(e.target.value)} placeholder="Last Name" />
            </div>
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="eu-email">Email Address</Label>
            <Input id="eu-email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="email@company.com" required />
          </div>
          <div className="space-y-1.5">
            <Label>Role</Label>
            <Select value={role} onValueChange={(v) => setRole(v as "admin" | "sales" | "marketing")}>
          ...
          <div className="space-y-1.5">
            <Label htmlFor="eu-erpEmployeeId">ERP Employee ID</Label>
            <Input
              id="eu-erpEmployeeId"
              value={erpEmployeeId}
              onChange={(e) => setErpEmployeeId(e.target.value)}
              placeholder="Paste ERP system generated ID (e.g. ERP-9912)"
            />
            <p className="text-[11px] text-muted-foreground">
              Explicitly enter the unique ID generated by the Vibhani India ERP system.
            </p>
          </div>
          {error && (
            <div className="text-sm text-destructive bg-destructive/10 border border-destructive/20 rounded-md px-3 py-2">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <div className="flex items-center gap-2 flex-wrap">
                                  ...
                                  {user.erpEmployeeId && (
                                    <Badge variant="outline" className="text-[10px] py-0 h-4 bg-amber-500/10 text-amber-500 border-amber-500/20 font-mono">
                                      ERP: {user.erpEmployeeId}
                                    </Badge>
                                  )}
                                </div>
